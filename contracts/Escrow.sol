...
contract Escrow {
    ...
    function getAgreement(uint256 index) external view returns (
        address bob,
        address alice,
        address arbitrator,
    ...
    function newAgreement(address _bob, address _alice, uint _bobExpectedAmount, uint _aliceExpectedAmount) external returns (uint) {
        require(_bob != _alice, "same buyer and seller");
        agreements.push(Agreement(_bob, _alice, msg.sender, _bobExpectedAmount, _aliceExpectedAmount, false, false));
        return agreements.length - 1;
    }

    function deposit(uint _id) external payable {
        if (msg.sender == agreements[_id].bob && msg.value == agreements[_id].aliceExpectedAmount) { //bob payed what alice expected
            agreements[_id].bobIn = true; 
        } else if(msg.sender == agreements[_id].bob && msg.value != agreements[_id].aliceExpectedAmount){
            revert("Bob has not paid what Alice expected");}
        else if (msg.sender == agreements[_id].alice && msg.value == agreements[_id].bobExpectedAmount) { //alice payed what bob expected
            agreements[_id].aliceIn = true;
        } else if(msg.sender == agreements[_id].alice && msg.value != agreements[_id].bobExpectedAmount){
            revert("Alice has not paid what Bob expected");
        }
    }

    function refund(uint _id) external payable{
        if (msg.sender == agreements[_id].bob && msg.value == agreements[_id].aliceExpectedAmount){
            agreements[_id].bobIn = false;
            payable(agreements[_id].bob).transfer(agreements[_id].aliceExpectedAmount);
        }else if  
         (msg.sender == agreements[_id].alice && msg.value == agreements[_id].bobExpectedAmount){
            agreements[_id].aliceIn = false;
            payable(agreements[_id].alice).transfer(agreements[_id].bobExpectedAmount);
        }else{
